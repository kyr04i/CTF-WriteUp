...
struct PlagueHeader {
    char key[MAX_HEADER_LENGTH]; // 128
    char value[MAX_HEADER_LENGTH]; // 128
};
...
void handle_request() {
    ...
    char start_line[MAX_START_LINE_SIZE]; // 1024
...
void handle_plague() {
    ...
    // take in the data
    char *plague_content = (char *)malloc(MAX_PLAGUE_CONTENT_SIZE); // 2048
...
void parse_headers() {
    ...
    ssize_t i = 0;
    char byteRead;
    char header_buffer[MAX_HEADER_DATA_SIZE]; // 1024
